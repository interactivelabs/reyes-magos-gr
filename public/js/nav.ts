export default function initNav() {
    ...
    const closeIfOutsideClick = ({element, elementButton, event}: {
      ...
      event: MouseEvent
        ...
        const isClickInside =
            element.contains(event.currentTarget as HTMLElement) || elementButton.contains(event.currentTarget as HTMLElement);
        if (!isClickInside) {
    ...
    document.addEventListener("click", (event) => {
        closeIfOutsideClick({element: adminMenuDropdown, elementButton: adminMenuButton, event: event});
        closeIfOutsideClick({element: mobileMenuContainer, elementButton: mobileMenuButton, event: event});
    });
}
