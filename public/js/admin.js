...
function createCategoryChip(category) {
  ...
  pill.className = "inline-flex items-center gap-2 rounded-pill bg-chip px-4 py-2 text-[13px] font-sans text-ink";
  const label = document.createElement("span");
  label.textContent = category;
  const removeButton = document.createElement("button");
  removeButton.type = "button";
  removeButton.className = "inline-flex h-4 w-4 shrink-0 items-center justify-center rounded-pill text-muted hover:text-accent";
  removeButton.innerHTML = `
    <span class="sr-only">Quitar categor\xEDa</span>
